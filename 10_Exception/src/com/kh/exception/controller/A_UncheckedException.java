...
public class A_UncheckedException {
	...
	public void method1() {
		...
		*/
		...
		try {
			...
			e.printStackTrace(); // 강제로 오류난 이력 보가자 할 때 실행하는 구문
		}
	...
	public void method3() {
		...
		*/
		...
		// 다중 catch 블럭 작성 가능
		// 각각의 예외 발생시 실행할 내용이 별개일 경우 세분화해서 다중 catch 블럭으로 기술하는게 적절함!
		
		try{
			int size = sc.nextInt(); // 문제가 되는 구문
			int[] arr = new int[size];
			System.out.println("100번 인덱스 값 : " + arr[100]);
		}catch(InputMismatchException e) { // 부모에 의해서 얘는 실행이 안됨. 순서가 중요함!!
			//, 정수가 아닌게 입력됐을듯?
			System.out.println("정수가 아닌 값을 입력하셨습니다.");
		}catch(RuntimeException e){ // 다형성 적용해서 부모타입예외클래스 작성 가능
									// 모든 자식 예외 발생시 다 받아서 처리할 수 있음
			System.out.println("예외가 발생되긴했음.. 배열의 크기가 잘못됐던가, 부적절한 인덱스가 제시됐던가");
		}
