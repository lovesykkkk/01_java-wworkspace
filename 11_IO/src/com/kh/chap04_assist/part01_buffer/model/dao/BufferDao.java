...
public class BufferDao {
	...
	public void fileSave() {
		...
		// 1. 기반스트림 먼저 생성
		//FileWriter fw = new FileWriter("c_buffer.txt"); //일단 예외처리 하지말기
		...
		*/
		...
		/*
		 * try(try블럭 내에서 사용할 스트림 객체 생성구문) { //알아서 try 다 끝나면 해당스트림 반납까지 해줌
		 * 
		 * } catch(예외클래스 e) {
		 * 
		 * }
	...
	public void fileRead() {
		...
		try(BufferedReader br = new BufferedReader(new FileReader("c_buffer.txt"))){
			...
			*/
			...
			String value = null;
			while((value = br.readLine()) != null) { // null과 비교할때는 equals 안써도됨
				System.out.println(value);
			}
			
			
		} catch (FileNotFoundException e) { //자식 => 위에있어야함. 없어도 된다.
			e.printStackTrace();
